...
- Формулы вывода площади и остатка не нормируются: применённые допущения обязаны возвращаться в ответе (`QUOTE-002`). Стандарт не навязывает свою арифметику.
- Свободная мощность декларативна, стандарт не требует её верификации (`CAP-002`). Верификация потребовала бы доступа в складскую систему оператора и закрыла бы протокол для всех, кто его не даёт.
...
166 тарифных вариантов из трёх реальных прайс-листов операторов разобраны в термины стандарта. Сами прайсы не публикуются: это данные их владельцев. Публично лежит результат — какие конструкции понадобились и что осталось непокрытым (`appendix/residual_gaps.csv`, 8 строк со статусом `open`, имена операторов обезличены).
